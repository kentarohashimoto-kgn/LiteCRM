...
| # | 論点 | 決定 | 設計への反映 |
|---|---|---|---|
| 1 | 連携範囲 | **freee会計 + 見積・請求**（人事労務は対象外／将来検討） | F-1〜F-4を対象。給与・勤怠連携はスコープ外 |
| 2 | 請求タイミング | **検収時に請求** | `billing_schedules` に検収状態を追加。**検収記録をトリガに請求書「下書き」を生成** |
| 3 | マスタの正 | **既存マスタ=freeeが正／今後の新規データ=CRMが正** | 初期は freee→CRM 一括インポート。以後の新規取引先・品目は **CRMで作成→freeeへpush** |
...
- **マスタ**: 初回は freee を正として CRM へ取り込む。**運用開始後の新規・更新は CRM を正**として freee へ反映（`freee_links` の対応表で名寄せ）。
...
1. **freeeのプロダクト構成**: 見積・請求は freee会計内の請求機能か、freee販売（旧・請求書サービス）か。使用中のAPIエンドポイントを確定したい。
