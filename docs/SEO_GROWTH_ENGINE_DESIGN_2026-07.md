...
- **同じ対象の承認待ちは常に1件**。既に承認待ちがあるときは新規作成せず、その行の期待値・根拠・施策タイプを最新に**上書きする**。DB側でも部分ユニーク索引 `uq_seo_proposals_open` で保証する（migration 0209）。

### 6.1.1 提案の単位は「記事1本」（migration 0209 / 2026-08-08 の修正）

**発見した不具合**: 承認画面に「狙う語『生成AI研修』を取る（新規記事の作成）」が4件並んでいた。承認待ち83件のうち71件が重複。原因は2つ。

1. **提案の単位がKW1語だった。** 0187/0189 で記事プラン（1記事＝メインKW1つ＋サブKW数語）を設計の単位にしたのに、生成側 `src/lib/seo/run-proposals.ts` がKWを1語ずつ提案していた。「生成AI研修」プランには26語ぶら下がっているので、承認するたびに別々の記事チケットが立つ＝薄い記事の量産になる設計だった。
2. **未承認のまま3日経つと同じ対象が再生成されていた。** 古い `pending_review` は残るので、日ごとに積み上がっていた。

さらに、0189 で RPC `seo_keyword_rankings` の返り値を変えた（`target_position` → `target_position_6m/12m`、`target_page` → `planned_url`）のに生成側が旧列名を読み続けていたため、**目標順位が常に5位固定・対象ページが常に空**になっていた。対象ページが空だと「同一ページに未完了施策を並走させない」ガード（§7）も効かない。

**修正後の設計**:

```
seo_keyword_rankings（KW × 順位 × 記事プラン）
      │ groupGapsByPlan()  ← src/lib/seo/plan-gap.ts（純関数・テスト対象）
      ▼
記事プラン単位の候補（1候補 = 1記事）
   ・期待値 = サブKW全部の追加クリック合計（1語ぶんで判断して見送るのを防ぐ）
   ・打ち手はプランの状態から一意に決まる
        対策ページが無い      → 新規記事
        ページはあるが圏外     → リライト
        ページがあり10位以内   → タイトル・説明文の改善（順位ではなくCTRの段階）
      ▼
seo_proposals（article_plan_id 付き・承認待ちは対象ごとに1件）
      ▼
承認 → seo_actions ＋ 指示書に「この記事で狙う語」の表を同梱
      ＋ content_ideas を起票し seo_article_plans.content_idea_id に紐付け
```

- 目標順位は **6ヶ月目標を優先**する。12ヶ月目標で期待値を出すと、半年先の理想値を「今月の見込み」として提示することになり過大になる。
- 記事プランに紐付いていないKW（GSCから拾った野良の語など）は、従来どおり1語1候補で扱う。
- 承認時の重複ガードは、対象ページに加えて**記事プラン**でも見る。新規記事は `target_page` が空なのでページだけでは重複を見つけられない。
