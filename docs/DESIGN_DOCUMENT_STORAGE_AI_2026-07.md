...
| **複製は2例外のみ** | (a) 発行物（公開スナップショット。編集と切り離すことが目的）(b) 証跡固定（契約書等、渡した時点の保全が必要な書類） |
...
### 3.1 ドライブ構成と権限（本部が統制、担当者が格納できる）

**共有ドライブ（マイドライブ禁止）を資料種別ごとに分け、権限はGoogleグループで束ねる**。個人アカウント直付与は原則禁止（退職・異動時の事故防止）。

| 共有ドライブ | 主フォルダ | 格納する物 | 権限（Googleグループ） | AI学習 |
|---|---|---|---|---|
| **SALES-資料庫** | `01_提案書/<顧客名>/`<br>`02_企画書/`<br>`03_研修資料/`<br>`04_営業ツール/` | 営業が日常で作る・使う資料 | `sales-all@`: **投稿者**（作成・編集可/削除・移動制限）<br>`sales-mgr@`: コンテンツ管理者<br>`backoffice@`: 閲覧者 | **対象** |
| **TECH-技術資料** | `01_製品仕様/` `02_導入手順/` `03_FAQ/` | AIヘルプの知識源となる技術文書 | `tech@`: 投稿者 / `sales-all@`: 閲覧者 | **対象** |
| **BO-バックオフィス** | `01_契約書類/` `02_請求/` `03_人事/` | 管理系 | `backoffice@`: 投稿者 / `sales-mgr@`: 閲覧(契約のみ) | **対象外**（既定でexcluded） |
| **CASE-案件フォルダ** | `<年度>/<顧客名>_<案件ID>/` | 案件ごとの成果物一式 | `sales-all@`: 投稿者 | 対象（案件スコープ） |
...
- **フォルダ規約 = `documents.category` の自動判定元**。取込時にファイルの親フォルダから種別を判定し、AI学習対象/対象外の既定も決まる（BO配下は自動でexcluded。個別上書き可）
...
- 案件作成時（または案件画面の「フォルダ作成」ボタン）に `ensureFolder()` で `CASE-案件フォルダ/<年度>/<顧客名>_<案件ID>/` を作成し、`opportunities` に `drive_folder_id` を保持
- 命名は機械可読（案件IDを含む）にし、リネームされてもIDで追跡可能

### 3.4 リンク健全性の維持（リンク方式の弱点対策）
...
| 埋め込みモデル | 外部API（候補: Voyage AI `voyage-3` 系 or OpenAI `text-embedding-3` 系。**Anthropic APIは埋め込み非提供**のため別途契約が必要）。次元は1024で確定しDDLに反映 | コスト目安: 資料1万ページ≒$1前後と軽微。※契約前に価格・日本語性能を比較評価する（実装フェーズ冒頭のスパイク項目） |
...
| **P2: 共有ドライブ運用** | 共有ドライブ・Googleグループの初期構築(運用手順書)、案件フォルダ自動作成、フォルダ→category自動判定、案件フォルダ自動取込 | §3の運用が回る | 小〜中 |
...
**P1の前提となる意思決定（残論点）**:
1. 埋め込みモデルの契約先（Voyage vs OpenAI — P4冒頭のスパイクで比較でも可）
2. 共有ドライブ再編の実施タイミング（既存ファイルの移動を伴うため、バックオフィスと日程調整が必要）
3. 「証跡固定」の対象範囲（契約書・発注書・検収書…どこまでをsnapshot必須にするか — バックオフィスと規程化）
4. 公開スナップショットの保持猶予の既定値（本書は30日で仮置き）
