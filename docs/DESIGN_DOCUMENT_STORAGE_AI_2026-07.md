...
|---|---|---|---|
| **CATORCE-資料庫** | `01_提案書` `02_企画書` `03_研修資料` `04_技術資料` `05_営業ツール` `09_テンプレート` | `sales-all@`: **投稿者**<br>`sales-mgr@`: コンテンツ管理者<br>`backoffice@`: 閲覧者 | **対象** |
| **CATORCE-案件** | `<顧客名>_<案件ID>`（CRMが自動作成。年度フォルダも作らずフラット） | `sales-all@`: 投稿者<br>`sales-mgr@`: コンテンツ管理者<br>※このドライブのみ**組織外共有を許可**: 協業パートナー等の外部ユーザーを**案件フォルダ単位**で追加する既存運用を継続（追加は管理者/コンテンツ管理者のみ・四半期棚卸し） | **対象**（案件スコープ。外部が投稿したファイルも取込まれる点に留意） |
| **CATORCE-BO** | `01_契約書類` `02_請求` `03_人事` | `backoffice@`: 投稿者<br>`sales-mgr@`: 閲覧（契約のみ運用注意） | **対象外**（既定でexcluded） |
