...
| **P2: 共有ドライブ運用+権限ガバナンス** ✅実装済 | 共有ドライブ・Googleグループの初期構築(運用手順書)、**フォルダ紐付け**(新規作成でなく既存フォルダのURLを貼る方式に変更)、フォルダ→category自動判定、**フォルダ内の新規ファイル自動取込**(夜間)、**§3.6の権限監査**(オフボーディング連動チェックリスト・権限スナップショット突合・NDA台帳・管理画面 `/app/settings/drive-audit`) | §3の運用が回る+漏れ検知が自動化 | 中 |
| **P3: 発行物ライフサイクル** | snapshot発行フロー(Codex側と連携)、unpublish+猶予+パージCron、再発行(版ズレ検知) | §4が回る・容量が発散しない | 中 |
| **P4: AIヘルプ第1期** ✅実装済 | pgvector/pgroonga有効化、抽出+チャンク+埋め込みパイプライン(夜間Cron `/api/cron/doc-index`)、ハイブリッド検索RPC `search_document_chunks`(ベクトル+日本語N-gramをRRF統合)、ヘルプQA UI `/app/assistant`(出典リンク付き) | 技術資料・営業資料へのQA | 中〜大 |
| ↑ 本文抽出の実現方式 | 新規npm依存を増やさず Drive API のみで完結: Google形式は `files.export`、PDF/Office は一時的にGoogle形式へ変換コピー(PDFはOCRも作用)→export→一時ファイル削除 | スキャンPDFも取込可能 | — |
