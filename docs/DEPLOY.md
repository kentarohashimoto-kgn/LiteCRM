...
1. 代表アカウントでログイン
   ...
   - パスワード: 管理者から別途安全な経路（1Password等）で共有された初期パスワード
     （初回ログイン後に必ず変更）
2. ダッシュボード・週次レビュー・商談一覧にデータが表示される
3. **設定 → メンバーを発行** で社員アカウントを作成できる
4. 外部営業アカウント（例: `tanaka@example.com`）でログインし直すと、
   **自分の担当案件のみ** 表示されることを確認

> ⚠️ パスワードを手順書・README・チャット等に平文で記載しないこと。
> 過去に記載していた初期パスワードは漏洩扱いとし、必ずローテーション（変更）する。
