...
**`pptxgenjs` の1つのみ**（スライド作成のPPTX生成）。それ以外はゼロ依存で構成している。

| 追加した依存 | 用途 | 足した理由 |
|---|---|---|
| `pptxgenjs` | スライド画像を1つの .pptx にまとめる（AL-226） | pptx は OOXML の ZIP パッケージで、自前で組むには ZIP 書き出しと複数XMLパートの整合が要る。**PowerPoint で開いて確かめられない開発環境**では、壊れたファイルを出しても気づけない。ここだけは実績のあるライブラリに任せる判断とした。追加の脆弱性は持ち込まない（依存は jszip / image-size のみ） |
