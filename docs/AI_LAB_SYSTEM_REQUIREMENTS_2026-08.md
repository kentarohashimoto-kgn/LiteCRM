...
| フェーズ | 内容 |
|---|---|
| **P1（MVP）** | 会社/利用者管理・Basic認証・個別ログイン・Claude 4種チャット（ストリーミング）・履歴・CRM動線 |
| **P2** | OpenAI チャット・画像生成・プリセット/アセット・モデル別利用可否 |
| **P3** | 利用集計ダッシュボード・予算/レート制限・プレビューリンク・一括発行UX改善 |

各フェーズ完了時に `docs/AI_LAB_TEST_SPEC_2026-08.md` の該当ケースを実施。
