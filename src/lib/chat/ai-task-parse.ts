...
const OUTPUT_SCHEMA = {
  ...
  properties: {
    title: {
      type: "string",
      description:
        "タスクの本文。期限表現・担当者指定・プロジェクト指定を除き、行動が分かる自然な日本語に組み立てる。" +
        "断片的な単語の羅列でも意味の通る一文にする（例:「和光紙器 動画アーカイブ 料金回答」→「和光紙器 動画アーカイブの料金回答」）。",
    },
    due_date: {
      type: "string",
      format: "date",
      description:
        "期限をYYYY-MM-DD形式で。「7月末」→その月の末日、「来週金曜」等の相対表現は今日の日付から解決。" +
        "「なるはや」「至急」「ASAP」→今日。指定が無ければ明日。",
    },
    assignee_name: {
      type: ["string", "null"],
      description:
        "担当者名。「担当◯◯」のほか「◯◯タスク」「◯◯さんの」「◯◯宛」等の表現も担当指定とみなす（メンバー一覧に近い名前がある場合）。" +
        "メンバー一覧の表記に最も近い1名。指定なし・「自分」の場合は null。",
    },
    project_name: {
      type: ["string", "null"],
      description:
    ...
    priority: {
      ...
      description: "至急/なるはや/ASAP/最優先→high、急がない/いつでも→low、それ以外→middle。",
    },
  },
...
export async function parseTaskWithAI(
  ...
  try {
    ...
    const response = await client.messages.create({
      ...
      system:
        "あなたはB2B営業CRMのタスク起票アシスタントです。日本語のゆらぎのある指示（断片的な単語の羅列を含む）から、タスクを正確に構造化します。" +
        "「タスク」「タスク追加」「追加して」「登録して」のような指示語はタイトルに含めないでください。" +
