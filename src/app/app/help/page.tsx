...
const DOCS = [
  ...
  },
  {
    ...
    points: ["ホーム／案件／顧客／分析／レビュー", "SEO集客／バックオフィス／人事／設定", "用語集（CRM・SEO）・ロール別の表示メニュー"],
      ...
      <Section title="よくある質問">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {[
            ...
            { q: "SEOの数字はいつ更新される？", a: "毎朝4時にSearch Console / GA4から自動取込します。急ぐときはSEO設定の「今すぐ取込を実行」。なおGoogle側の仕様でデータは2〜3日遅れで届くため「昨日の数字」は存在しません。" },
            { q: "SEOの提案はどれからやればいい？", a: "改善提案は期待値（円換算）の大きい順に並んでいるので、上から承認すれば大きく間違えません。承認後は施策実行の指示書（プロンプト）を別AIやHP担当に渡し、公開したページのURLを貼って「反映を記録」を押してください（記録しないと効果検証が始まりません）。" },
          ].map((f) => (
