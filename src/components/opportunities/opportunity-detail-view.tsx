 ...
 * スライドオーバー（インターセプトルート）の両方から再利用する。
 *  - inPane=true のときは一覧へ戻るリンクを省く（ペインの閉じるで戻るため）。
 */
export async function OpportunityDetailView({ id, inPane = false, saved, error }: { id: string; inPane?: boolean; saved?: string; error?: string }) {
  const ws = await getWorkspaceForOpportunity(id);
  const o = getOpportunity(ws, id);
  if (!o) notFound();
  ...
  return (
    <div>
      ...
      {error && (
        <div className="mb-4 rounded-lg border border-rose-200 bg-rose-50 px-4 py-2.5 text-sm text-rose-600">{error}</div>
      )}
      {saved && (
        <div className="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-sm text-emerald-700">✓ {SAVED_MSG[saved] ?? "保存しました"}</div>
      )}
