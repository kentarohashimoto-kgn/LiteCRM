...
/**
 * 商談メモの本体。フルページ（/app/opportunities/[id]/meetings/[mid]）と、
 * 案件一覧・カレンダーのスライドオーバー（インターセプトルート）の両方から再利用する。
 *  - 商談中に「顧客情報・事前リサーチ・営業戦略」を参照しながらメモを取れるよう、
 *    左=メモ入力(商談を編集) / 右=参照パネル の2カラム構成。
 *  - inPane=true のときは案件へのリンクをペイン内遷移（?mid なしの同一案件URL）にし、
 *    保存後の戻り先も return_to でペインのURLへ固定する（カレンダーを保持したまま編集できる）。
 */
export async function MeetingDetailView({
  ...
  const ws = await getWorkspaceForOpportunity(oppId);
  const meeting = getMeeting(ws, mid);
  if (!meeting || meeting.opportunity_id !== oppId) notFound();
  const opp = getOpportunity(ws, oppId);
  ...
  // 案件へのリンク先とアクション後の戻り先。ペイン内では同じスライドオーバーに留める。
  const oppHref = `/app/opportunities/${oppId}`;
  const returnTo = inPane ? `${oppHref}?mid=${mid}&saved=1` : null;

  return (
    <div>
      <div className="flex items-center justify-between gap-3 mb-3">
        <Link href={oppHref} className="inline-flex items-center gap-1 text-sm text-ink/50 hover:text-ink">
          <ChevronLeft size={16} /> {inPane ? "この案件の詳細を見る" : "案件へ戻る"}
        </Link>
        {canDeleteMeeting && <DeleteMeetingButton meetingId={meeting.id} opportunityId={meeting.opportunity_id} returnTo={inPane ? oppHref : undefined} />}
      </div>
      {/* データ階層: 顧客 > 案件 > 商談(いまここ) */}
      <DataPath
        items={[
          ...(meeting.account ? [{ level: "account" as const, name: meeting.account.name, href: `/app/accounts/${meeting.account.id}` }] : []),
          { level: "opportunity", name: opp?.name, href: oppHref },
      ...
      <div className={inPane ? "space-y-5" : "grid grid-cols-1 lg:grid-cols-3 gap-5 items-start"}>
        {/* 左: 商談メモの入力(商談中はここに書く) */}
        <div className={inPane ? "space-y-5" : "lg:col-span-2 space-y-5"}>
          ...
          <Section
            title="AI要約"
            action={<AiSummaryButton meetingId={meeting.id} opportunityId={oppId} hasMinutes={Boolean(meeting.minutes_detail && meeting.minutes_detail.trim().length >= 30)} />}
          >
            {meeting.ai_summary ? (
              <div>
        ...
        {/* 右: 参照パネル(スクロールに追従)。商談中にカンペとして見る場所
            （ペイン内は幅が狭いので2カラムにせず、メモの下に積む） */}
        <div className={inPane ? "space-y-4" : "space-y-4 lg:sticky lg:top-4 lg:max-h-[calc(100vh-2rem)] lg:overflow-y-auto lg:pr-1"}>
          <Section title="案件サマリ" className={entityBorder("opportunity")} action={
            <Link href={oppHref} className="inline-flex items-center gap-1 text-[11px] text-teal-deep hover:underline">案件を開く <ExternalLink size={11} /></Link>
          }>
