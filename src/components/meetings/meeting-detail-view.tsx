 ...
 */
/**
 * 商談メモ画面の本体。フルページと案件一覧のスライドオーバー（インターセプト）で共用。
 *  - inPane=true のときは「案件へ戻る」リンクを省く（ペイン側で戻る導線を出す）。
 */
export async function MeetingDetailView({ id, mid, inPane = false, saved }: { id: string; mid: string; inPane?: boolean; saved?: string }) {
  const params = { id, mid };
