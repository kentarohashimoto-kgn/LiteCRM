...
| payment_notice_number | query | いいえ | string | 支払通知書番号 |
...
| start_payment_notice_date | query | いいえ | string(date) | 支払通知書日付の開始日 |
| end_payment_notice_date | query | いいえ | string(date) | 支払通知書日付の終了日 |
...
- payment_notices (必須): array[object]
  配列の要素:
    - id (必須): integer(int64) - 支払通知書ID (最小: 1, 最大: 9223372036854775000)
    - company_id (必須): integer(int64) - 事業所ID (最小: 1, 最大: 9223372036854775000)
    - payment_notice_number (必須): string - 支払通知書番号
    - subject (必須): string - 件名
    - template_id (任意): integer(int64) - 帳票テンプレートID (最小: 1, 最大: 9223372036854775000)
    - payment_notice_date (必須): string(date) - 支払通知書日付 (パターン: ^[0-9]{4}-[0-9]{2}-[0-9]{2}$)
...
- payment_notice_note (任意): string - 備考
- memo (任意): string - 社内メモ
...
- lines (必須): array[object] - 支払通知書の明細行
  配列の要素:
...
- payment_notice (必須): object
  ...
  - collects_on (任意): string(date) - 支払予定日 (パターン: ^[0-9]{4}-[0-9]{2}-[0-9]{2}$)
  - payment_condition (任意): string - 支払方法
  - payment_notice_note (必須): string - 備考
  - memo (必須): string - 社内メモ
  - sending_status (必須): string - 送付ステータス（sent: 送付済み、 unsent: 送付待ち） (選択肢: sent, unsent)
  - cancel_status (必須): string - 取消済み（canceled: 該当する、 uncanceled: 該当しない） (選択肢: canceled, uncanceled)
...
### GET /payment_notices/templates

操作: 使用可能な支払通知書の帳票テンプレート一覧の取得


説明: 使用可能な支払通知書の帳票テンプレート一覧を返します。
...
### GET /payment_notices/{id}

操作: 支払通知書の取得


説明: 指定されたIDの支払通知書を返します。
...
### PUT /payment_notices/{id}/cancel

操作: 支払通知書の取消


説明: 指定された支払通知書を取消状態にします。 取引が紐づいている場合は取引も削除されます。
...
### PUT /payment_notices/{id}/uncancel

操作: 取消された支払通知書の復元


説明: 指定された取消済み支払通知書を復元します。
