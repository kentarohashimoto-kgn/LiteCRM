-- 営業レントゲンv2: 既存顧客エンジン(全期間ベースの母数・FU実績)とサブスク/ストック売上の指標を追加。
-- 追加: booked.revenue_stock / 'base'オブジェクト(期間非依存の構造指標)。
-- ストック売上の判定: サブスク的商材(名前に 顧問/サブスク/月額/保守/運用/SES) or recurring請求のある案件。
