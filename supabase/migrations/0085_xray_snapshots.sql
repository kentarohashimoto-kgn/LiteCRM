-- 営業レントゲンの分析スナップショット(保存・履歴)と月次自動保存の基盤。
-- 1) xray_metrics_for_tenant: テナント明示指定版RPC。cron(service_role)専用。
--    一般ユーザーからは実行不可(他テナント指定を防ぐためrevoke)。
-- 2) xray_metrics: 認証ユーザー向けの薄いラッパーに変更(ロジックの二重管理を解消)。
-- 3) xray_snapshots: 分析結果の保存(手動/月次自動)。payloadにRPC結果のjsonbを丸ごと保持。

create or replace function public.xray_metrics_for_tenant(p_tenant uuid, p_start date, p_end date, p_cmp_start date, p_cmp_end date)
returns jsonb
language plpgsql
...
as $$
declare
  v uuid[] := array[p_tenant];
  result jsonb;
begin
  if p_tenant is null then return '{}'::jsonb; end if;
  ...
  booked as (
    select r.tag,
      ...
      -- ストック売上: サブスク的商材(顧問/月額/保守/運用/SES/サブスク) or recurring請求のある案件
      coalesce(sum(o.amount) filter (where
        exists (select 1 from products pr where pr.id = o.primary_product_id and pr.name ~ '顧問|サブスク|月額|保守|運用|SES')
        or exists (select 1 from billing_schedules b where b.opportunity_id = o.id and b.kind = 'recurring')
      ),0)::numeric as revenue_stock
    from ranges r
  ...
  merged as (
    select r.tag,
      jsonb_build_object(
        ...
        'revenue_stock', bk.revenue_stock,
  ...
  -- 期間に依存しない構造指標(既存顧客エンジンの母数・全期間実績)
  base as (
    select jsonb_build_object(
      'won_accounts', (select count(distinct account_id) from opportunities where tenant_id=any(v) and status='won' and deleted_at is null),
      'repeat_accounts', (select count(*) from (
        select account_id from opportunities where tenant_id=any(v) and status='won' and deleted_at is null
        group by account_id having count(*) >= 2) x),
      'avg_won_price', (select coalesce(avg(amount),0) from opportunities where tenant_id=any(v) and status='won' and deleted_at is null and amount > 0),
      'fu_cases_total', (select count(*) from fu_cases where tenant_id=any(v)),
      'fu_meets_total', (select count(*) from fu_meetings where tenant_id=any(v)),
      'fu_held_total', (select count(*) from fu_meetings where tenant_id=any(v) and (schedule_status='done' or held_on is not null)),
      'fu_prop_total', (select count(*) from fu_meetings where tenant_id=any(v) and proposal_done),
      'fu_upsell_total', (select count(*) from fu_meetings where tenant_id=any(v) and upsell_status is not null and upsell_status not in ('none')),
      'revenue_all', (select coalesce(sum(amount),0) from opportunities where tenant_id=any(v) and status='won' and deleted_at is null),
      'revenue_stock_all', (select coalesce(sum(o.amount),0) from opportunities o where o.tenant_id=any(v) and o.status='won' and o.deleted_at is null
        and (exists (select 1 from products pr where pr.id = o.primary_product_id and pr.name ~ '顧問|サブスク|月額|保守|運用|SES')
          or exists (select 1 from billing_schedules b where b.opportunity_id = o.id and b.kind = 'recurring'))),
      'mrr', (select coalesce(sum(amount),0) from billing_schedules where tenant_id=any(v) and kind='recurring' and coalesce(sub_status,'active')='active'),
      'recurring_contracts', (select count(distinct opportunity_id) from billing_schedules where tenant_id=any(v) and kind='recurring' and coalesce(sub_status,'active')='active')
    ) as body
  ),
  tg as (
    select jsonb_build_object(
...
begin
  ...
  return public.xray_metrics_for_tenant(v[1], p_start, p_end, p_cmp_start, p_cmp_end);
end $$;

-- テナント指定版は一般ユーザーから実行不可に(security definerの内部呼び出しとservice_roleのみ)
revoke execute on function public.xray_metrics_for_tenant(uuid, date, date, date, date) from public, anon, authenticated;
grant execute on function public.xray_metrics_for_tenant(uuid, date, date, date, date) to service_role;

-- 分析スナップショット
create table if not exists xray_snapshots (
  id uuid primary key default gen_random_uuid(),
  tenant_id uuid not null references tenants(id) on delete cascade,
  kind text not null default 'manual' check (kind in ('manual','monthly')),
  label text,
  taken_at timestamptz not null default now(),
  taken_on date not null default (now() at time zone 'Asia/Tokyo')::date,
  period_start date not null,
  period_end date not null,
  cmp_start date not null,
  cmp_end date not null,
  payload jsonb not null,
  created_by uuid references profiles(id),
  created_at timestamptz not null default now()
);
create index if not exists idx_xray_snapshots_tenant_taken on xray_snapshots(tenant_id, taken_at desc);
-- 月次自動保存は同一対象月に1件のみ(冪等)
create unique index if not exists uq_xray_snapshots_monthly
  on xray_snapshots(tenant_id, period_start) where kind = 'monthly';

alter table xray_snapshots enable row level security;
create policy xray_snap_select on xray_snapshots for select
  using (tenant_id in (select current_tenant_ids()));
create policy xray_snap_insert on xray_snapshots for insert
  with check (tenant_id in (select current_tenant_ids()) and can_edit_role(tenant_id) and created_by = auth.uid());
create policy xray_snap_delete on xray_snapshots for delete
  using (tenant_id in (select current_tenant_ids())
